...
Arc.prototype.getTangentAtLength = function( length ) {
	...
	const p1 = this.getPointAtLength( length ) ;

	let p2 ;

	if ( length < this.length - pointDist ) {
		p2 = this.getPointAtLength( length + pointDist ) ;
	}
	else {
		p2 = this.getPointAtLength( length - pointDist ) ;
	}
...
Arc.prototype.getPointAndTangentAtLength = function( length ) {
